...
class SimpleRouter < Trema::Controller
  ...
  def packet_in( datapath_id, message )
    ...
    when Pio::Parser::IPv4Packet
      ...
      fail 'Failed to manage packet_in data type'
    end
  end
  ...
  def handle_ipv4( datapath_id, message )
    ...
    elsif message.data === Pio::Icmp::Request
      handle_icmpv4_echo_request( datapath_id, message )
    else
      fail "Faild to manage icmp type"
    end
  end
  ...
  def forward( datapath_id, message )
    p next_hop = resolve_next_hop( message.data.ip_destination_address )

    p interface = @interfaces.find_by_prefix( next_hop )
